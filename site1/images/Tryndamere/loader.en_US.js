...
// parts of the URL.
setRegion(regionFromQueryParam);
} else if(!region_from_cookie) {
//Did we get a region from the URL?
if(region_from_url != null){
setRegion(region_from_url);
}else{//Use the default
setRegion(sw_cfg.default_region.toLowerCase());
}
...
!function(a){"use strict";function b(){return a.Performance&&a.performance instanceof a.Performance}function c(){return a.performance.now()}function d(){return(new Date).getTime()}function e(a){return o.call(a,0)}function f(){a.ga||!function(a,b,c,d,e,f,g){a.GoogleAnalyticsObject=e,a[e]=a[e]||function(){(a[e].q=a[e].q||[]).push(arguments)},a[e].l=1*new Date,f=b.createElement(c),g=b.getElementsByTagName(c)[0],f.async=1,f.src=d,g.parentNode.insertBefore(f,g)}(window,document,"script","//www.google-analytics.com/analytics.js","ga")}function g(){var b=new RegExp("riot_cpa=([^;]*)"),c=b.exec(a.document.cookie);return c?"true"===c[1]:!1}function h(b,c){a.ga("create",b,{name:c})}function i(a){return a.indexOf("euw")>=0||a.indexOf("eune")>=0?g()?!0:!1:!0}function j(a){return new k(a)}function k(){this.init.apply(this,arguments)}var l,m=a.Riot=a.Riot||{},n=0,o=Array.prototype.slice;k.prototype={init:function(a){var b="ga"+1*new Date+ ++n;i(window.location.hostname)&&(f(),h(a,b)),this.name=b},time:function(a,b,c){var d=l(),e=this;return function(){e.send("timing",a.substr(0,150),b.substr(0,500),Math.round(l()-d),c.substr(0,500))}},call:function(){if(i(window.location.hostname)){var b=e(arguments);-1===b[0].indexOf(".")&&(b[0]=this.name+"."+b[0]),a.ga.apply(null,b)}},send:function(){var a=e(arguments);a.unshift("send"),this.call.apply(this,a)},getGA:function(){return a.ga},getName:function(){return this.name}},l=b()?c:d,k.create=j,m.Tracker=k}(window);(function (win) {
...
return $.when($(oldState).show().promise());
}
...
_show: function () {
this.showTab(this.activeSection || this.defaultSection);
},
_hide: function () {
...
var Sandworm = Riot.Sandworm,
ManageMenu = Sandworm.ManageMenu,
twigId = "management_change_riot_alpha_signup",
renderFn = "_renderChangeRiotAlphaSignupTab",
tabId = "change_riot_alpha_signup";
if (Sandworm) {
Sandworm.isRiotAlphaSignUpEnabledForUser = function() {
var thisRegion = Riot.Sandworm.getRegion();
var alphaRegions = sw_cfg.lcu_alpha_regions;
for(var i = 0; i < alphaRegions.length; i++) {
if(alphaRegions[i] === thisRegion) {
return true;
}
}
return false;
};
if (Sandworm.isRiotAlphaSignUpEnabledForUser()) {
Sandworm.setTwig('management_as', [
"<li class='tab' data-goto-ref='change_riot_alpha_signup'>{{ ACCT_RIOT_ALPHA }}</li>"
]);
}
}
if (ManageMenu) {
...
l10n: this.l10n,
host: this.config.sw_cfg.host
}, Sandworm.ChangeRiotAlphaSignup);
return section.render(this.tabContainer);
};
}
Sandworm.ChangeRiotAlphaSignup = {
...
Riot.cssInject("pvpnet-alphasignup");
$('.sw-riotalpha-signup', container).click($.proxy(this.onSignUp, this));
$('.sw-riotalpha-verifylink', container).click($.proxy(this.onVerifyClick, this));
$('.sw-riotalpha-retrylink', container).click($.proxy(this.onRetryClick, this));
},
_activate: function () {
var self = this;
self.retry = "_activate";
return self.setState("loading").
pipe(function () {
return self.bridge.send({
type: "loadstatus",
action: "/alpha/get-state",
method: "GET"
});
}).
pipe(function (data) {
...
if (data.link) {
$('.sw-riotalpha-link', this.content).attr("href", data.link);
}
return state;
}, function (data) {
var state;
try {
state = data.error.response.error;
} catch(e) {}
return ('ratelimited' === state) ? state : 'error';
...
return self.bridge.send({
type: "changestatus",
action: "/alpha/set-pending-state",
method: "POST"
});
}).
pipe(function (data) {
...
"<div id='sw-riotalpha'>",
"<div class='sw-riotalpha-hero'>",
"<div class='sw-riotalpha-hero-content'>",
"<h3>{{ ACCT_RIOT_ALPHA_LCU }}</h3>",
"<p>{{ ACCT_RIOT_ALPHA_CAPTION }}</p>",
"</div>",
"</div>",
"<div class='sw-managesection-state-not' style='display: none'>",
"<div class='sw-riotalpha-status'>",
"<a href='#' class='sw-riotalpha-signup'>",
"<span >{{ ACCT_RIOT_ALPHA_SIGN_UP }}</span>",
"</a>",
"<a href='#' class='sw-riotalpha-link' target='_blank'>",
"{{ ACCT_RIOT_ALPHA_MORE_INFO }} <i></i>",
"</a>",
"</div>",
"<div class='sw-riotalpha-desc'>{{ ACCT_RIOT_ALPHA_DESC_NOT }}</div>",
"</div>",
"<div class='sw-managesection-state sw-managesection-state-loading' data-state='loading' style='display: none'>",
"<div class='sw-riotalpha-spinner'></div>",
"</div>",
"<div class='sw-managesection-state sw-managesection-state-verify' data-state='verify' style='display: none'>",
"<div class='sw-riotalpha-status'>{{ ACCT_RIOT_ALPHA_STATUS_VERIFY }}</div>",
"<div class='sw-riotalpha-desc'>{{ ACCT_RIOT_ALPHA_DESC_VERIFY }}</div>",
"</div>",
"<div class='sw-managesection-state sw-managesection-state-ratelimited' data-state='ratelimited' style='display: none'>",
"<div class='sw-riotalpha-status'>{{ ACCT_RIOT_ALPHA_STATUS_ERROR }}</div>",
"<div class='sw-riotalpha-desc'>{{ ACCT_RIOT_ALPHA_DESC_RATELIMITED }}</div>",
"</div>",
"<div class='sw-managesection-state sw-managesection-state-error' data-state='error' style='display: none'>",
"<div class='sw-riotalpha-status'>{{ ACCT_RIOT_ALPHA_STATUS_ERROR }}</div>",
"<div class='sw-riotalpha-desc'>{{ ACCT_RIOT_ALPHA_DESC_ERROR }}</div>",
"</div>",
"<div class='sw-managesection-state-pending' style='display: none'>",
"<div class='sw-riotalpha-status'>",
"<span>{{ ACCT_RIOT_ALPHA_STATUS_PENDING }}</span>",
"<a href='#' class='sw-riotalpha-link' target='_blank'>",
"{{ ACCT_RIOT_ALPHA_MORE_INFO }} <i></i>",
"</a>",
"</div>",
"<div class='sw-riotalpha-desc'>{{ ACCT_RIOT_ALPHA_DESC_PENDING }}</div>",
"</div>",
"<div class='sw-managesection-state-active' style='display: none'>",
"<div class='sw-riotalpha-status'>",
"<span>{{ ACCT_RIOT_ALPHA_STATUS_ACTIVE }}</span>",
"<a href='#' class='sw-riotalpha-link'>",
"{{ ACCT_RIOT_ALPHA_DOWNLOAD }} <i></i>",
"</a>",
"</div>",
"<div class='sw-riotalpha-desc'>{{ ACCT_RIOT_ALPHA_DESC_ACTIVE }}</div>",
"</div>",
"</div>"
]);
}(jQuery));
Riot.cssPrepare("pvpnet-alphasignup","#sw-riotalpha{background:url('https://account.leagueoflegends.com/bundles/riotsandworm/img/lcu/logo.png') no-repeat right bottom;min-height:330px;padding-bottom:40px;position:relative}#sw-riotalpha h2{font-size:12px;font-weight:normal;margin:0 0 10px 0;text-transform:uppercase;line-height:14px;white-space:nowrap}#sw-riotalpha .sw-riotalpha-hero{background:url('https://account.leagueoflegends.com/bundles/riotsandworm/img/lcu/sprite.en_US.png') no-repeat left top;height:95px;width:443px;overflow:hidden;position:relative}#sw-riotalpha .sw-riotalpha-hero .sw-riotalpha-hero-content{position:absolute;left:443px}#sw-riotalpha .sw-riotalpha-signup{background:url('https://account.leagueoflegends.com/bundles/riotsandworm/img/lcu/sprite.en_US.png') no-repeat 0 -96px;width:118px;height:45px;position:relative;overflow:hidden;display:inline-block}#sw-riotalpha .sw-riotalpha-signup:hover{background-position:-119px -96px}#sw-riotalpha .sw-riotalpha-signup:active{background-position:-238px -96px}#sw-riotalpha .sw-riotalpha-signup>span{position:absolute;left:118px}#sw-riotalpha .sw-riotalpha-spinner{background:url('https://account.leagueoflegends.com/bundles/riotsandworm/img/lcu/spinner.gif') no-repeat left top;position:absolute;top:32px;width:32px;height:32px;right:18px}#sw-riotalpha .sw-riotalpha-status{color:#D4C17D;position:absolute;top:34px;line-height:14px;right:18px;text-align:right;font-weight:bold;width:118px;}#sw-riotalpha .sw-managesection-state-not .sw-riotalpha-status{text-align:center;top:21px}#sw-riotalpha .sw-riotalpha-link{color:#00BCD4 !important;font-size:10px;font-weight:normal;display:block}#sw-riotalpha .sw-riotalpha-link>i{display:inline-block;width:10px;height:10px;background:url('https://account.leagueoflegends.com/bundles/riotsandworm/img/lcu/sprite.en_US.png') right bottom no-repeat transparent;vertical-align:middle}#sw-riotalpha .sw-riotalpha-desc{margin:24px 40px 0}#sw-riotalpha .sw-riotalpha-desc>h3{font-size:24px;font-weight:normal;line-height:1.5;border-bottom:1px solid #585858;margin-bottom:11px}#sw-riotalpha .sw-riotalpha-desc>p{font-size:13px;line-height:17px}");
...
if (window.sw_cfg && window.sw_cfg.rso_regions && Riot.Sandworm.getRegion()) {
var region = Riot.Sandworm.getRegion(),
lang = (Riot.Sandworm.getLang() || 'en').substring(0,2),
rsoRegions = window.sw_cfg.rso_regions,
origin = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ':' + window.location.port: '');
if (rsoRegions.indexOf(region) > -1 && origin != sw_cfg.host
&& menu != 'regions' && menu != 'login') {
window.location.href = sw_cfg.host + '/' + region + '/' + lang + '/protect';
return;
}
}
...
show_account:true,
// should we display the sandworm navigation bar?
...
register_link: 'https://signup.{region}.leagueoflegends.com/{lang}',
...
if (options.show_navigation_bar) {
...
var regionURL = options.register_link;
regionURL = regionURL.replace('{region}', Riot.Sandworm.getRegion());
regionURL = regionURL.replace('{lang}', Riot.Sandworm.getLang().substr(0,2));
